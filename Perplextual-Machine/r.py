...
with open(fname,'w') as w:
	...
	if mode == 0:
		...
		def c_000(y):
			...
			dex0 = dt.datetime(2000+(y400),4,11)
			...
			for i in range (3,10):
				if c.weekday(y400,12,i) == c.SATURDAY:
					dtechno = dt.datetime(2000+(y400),12,i)
			for i in range (10,17):
				if c.weekday(y400,12,i) == c.SATURDAY:
					dyear = dt.datetime(2000+(y400),12,i)
			...
			dwcalrd = dt.datetime(2000+(y400),10,23)
			dglobal = dt.datetime(2000+(y400),7,10)
			for i in range (13,20):
				if c.weekday(y400,1,i) == c.SATURDAY:
					daaexpo0 = dt.datetime(2000+(y400),1,i)
			for i in range (14,21):
				if c.weekday(y400,1,i) == c.SUNDAY:
					daaexpo1 = dt.datetime(2000+(y400),1,i)
			for i in range (1,8):
				if c.weekday(y400,3,i) == c.SUNDAY:
			...
			for i in range (22,29):
				if c.weekday(y400,8,i) == c.THURSDAY:
					readingA1 = dt.datetime(2000+(y400),8,i)
			for i in range (23,30):
				if c.weekday(y400,8,i) == c.FRIDAY:
					readingA2 = dt.datetime(2000+(y400),8,i)
			for i in range (24,31):
				if c.weekday(y400,8,i) == c.SATURDAY:
					readingA3 = dt.datetime(2000+(y400),8,i)
			for i in range (25,32):
				if c.weekday(y400,8,i) == c.SUNDAY:
					readingA4 = dt.datetime(2000+(y400),8,i)
			for i in range (21,28):
				if c.weekday(y400,11,i) == c.THURSDAY:
					readingB1 = dt.datetime(2000+(y400),11,i)
			for i in range (22,29):
				if c.weekday(y400,11,i) == c.FRIDAY:
					readingB2 = dt.datetime(2000+(y400),11,i)
			for i in range (23,30):
				if c.weekday(y400,11,i) == c.SATURDAY:
					readingB3 = dt.datetime(2000+(y400),11,i)
			for i in range (24,31):
				if c.weekday(y400,12,i) == c.SUNDAY:
					readingB4 = dt.datetime(2000+(y400),11,i)
			...
			for i in range(13,20):
				if c.weekday(y400,1,i) == c.SATURDAY:
					w.write(("    %s - [%s] - Sat - 01 - %02d  : AA Expo %04d (California, Sat - Sun Before MLK) Day 01\n") % (daaexpo0.strftime("%j"),sat_number,i,y10k))
			for i in range(14,21):
				if c.weekday(y400,1,i) == c.SUNDAY:
					w.write(("    %s - [%s] - Sun - 01 - %02d  : AA Expo %04d (California, Sat - Sun Before MLK) Day 02\n") % (daaexpo1.strftime("%j"),sun_number,i,y10k))
			for i in range (15,22):
				if c.weekday(y400,1,i) == c.MONDAY:
			...
			for i in range (1,8):
				if c.weekday(y400,3,i) == c.SUNDAY:
					w.write(("    %s - [%s] - Sun - 03 - %02d  : Super Sunday %04d\n") % (dsunday.strftime("%j"),sun_number,i,y10k))
			...
			w.write(("    %s - [%d] - %s - 04 - 11  : Modern International Standards and Release Day\n") % (dex0.strftime("%j"),(c.weekday(y400,4,3)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,4,11)]))
			...
			w.write(("    %s - [%d] - %s - 07 - 10  : Global Music Release Day\n") % (dglobal.strftime("%j"),(c.weekday(y400,7,10)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,7,10)]))
			...
			for i in range (22,29):
				if c.weekday(y400,8,i) == c.THURSDAY:
					w.write(("    %s - [%s] - Thu - 08 - %02d  : World Reading Championship %04d - Cohort A Day 01\n") % (readingA1.strftime("%j"),thu_number,i,y10k))
			for i in range (23,30):
				if c.weekday(y400,8,i) == c.FRIDAY:
					w.write(("    %s - [%s] - Fri - 08 - %02d  : World Reading Championship %04d - Cohort A Day 02\n") % (readingA2.strftime("%j"),fri_number,i,y10k))
			for i in range (24,31):
				if c.weekday(y400,8,i) == c.SATURDAY:
					w.write(("    %s - [%s] - Sat - 08 - %02d  : World Reading Championship %04d - Cohort A Day 03\n") % (readingA3.strftime("%j"),sat_number,i,y10k))
			for i in range (25,32):
				if c.weekday(y400,8,i) == c.SUNDAY:
					w.write(("    %s - [%s] - Sun - 08 - %02d  : World Reading Championship %04d - Cohort A Day 04\n") % (readingA4.strftime("%j"),sun_number,i,y10k))
			...
			w.write(("    %s - [%d] - %s - 10 - 01  : %04d Gregorian Calendar Default Month Formula Point\n") % (dmus1.strftime("%j"),(c.weekday(y400,10,1)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,10,1)],y10k))
			...
			w.write(("    %s - [%d] - %s - 10 - 14  : World's Standards Day (Original Date)\n") % (dstandardori.strftime("%j"),(c.weekday(y400,10,14)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,10,14)]))
			...
			w.write(("    %s - [%d] - %s - 10 - 15  : World Calendar Day\n") % (dun002G.strftime("%j"),(c.weekday(y400,10,15)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,10,15)]))
			...
			w.write(("    %s - [%d] - %s - 10 - 16  : %04d Worldwide Calendar's Announcement Day\n") % (dspt.strftime("%j"),(c.weekday(y400,10,16)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,10,16)],(y10k+1)%10000))
			...
			w.write(("    %s - [%d] - %s - 10 - 23  : %04d Worldwide Calendar's Release Day\n") % (dwcalrd.strftime("%j"),(c.weekday(y400,10,23)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,10,23)],(y10k+1)%10000))
			...
			for i in range (21,28):
				if c.weekday(y400,11,i) == c.THURSDAY:
					w.write(("    %s - [%s] - Thu - 11 - %02d  : World Reading Championship %04d Cohort B Day 01\n") % (readingB1.strftime("%j"),thu_number,i,y10k))
			for i in range (22,29):
				if c.weekday(y400,11,i) == c.FRIDAY:
					w.write(("    %s - [%s] - Fri - 11 - %02d  : World Reading Championship %04d Cohort B Day 02\n") % (readingB2.strftime("%j"),fri_number,i,y10k))
			for i in range (23,30):
				if c.weekday(y400,11,i) == c.SATURDAY:
					w.write(("    %s - [%s] - Sat - 11 - %02d  : World Reading Championship %04d Cohort B Day 03\n") % (readingB3.strftime("%j"),sat_number,i,y10k))
			for i in range (24,31):
				if c.weekday(y400,11,i) == c.SUNDAY:
					w.write(("    %s - [%s] - Sun - 11 - %02d  : World Reading Championship %04d Cohort B Day 04\n") % (readingB4.strftime("%j"),sun_number,i,y10k))
			...
			for i in range (3,10):
				if c.weekday(y400,12,i) == c.SATURDAY:
					w.write(("    %s - [%s] - Sat - 12 - %02d  : International Kailedoscope Awards %04d - Technoworld\n") % (dtechno.strftime("%j"),sat_number,i,y10k))
			for i in range (10,17):
				if c.weekday(y400,12,i) == c.SATURDAY:
					w.write(("    %s - [%s] - Sat - 12 - %02d  : International Kaleidoscope Awards %04d - Best Of Year\n") % (dyear.strftime("%j"),sat_number,i,y10k))
			for i in range (17,24):
				if c.weekday(y400,12,i) == c.SATURDAY:
		...
		def lunar_bc(y400,m,d):
			...
			if ((dic >= 0 and dic <= 5 + (28169441/70499183)) or (dic >= 1834547211/70499183 and dic < (2081882250/70499183))) and (1 <= m <= 2):
				w.write(("    %s - [%d] - %s - %02d - %02d  : Chinese New Year's Day [HD] %04d [%s]\n") % ((day_of_year_list.strftime("%j")),(c.weekday(y400,m,d)+2-week_system_ordinal)%7+1,c.day_abbr[c.weekday(y400,m,d)],m,d,(y10k+2698)%10000,dic))
		...
		w.write("Note : All year value(s) in mod 10^64 (!) Except Huang Di [HD] Calendar\n\n")
